
/**
 * LevelSelectionPanel
 * 
 * Displays a single map selection screen 
 *
 * @author Julius Gauldie
 * @version 14/08/25
 */

import java.awt.*;
import javax.swing.*;
import javax.swing.border.EmptyBorder;

public class LevelSelectionPanel extends JPanel 
{
    // Manages reading and writing high scores
    private HighScoreManager scoreManager = new HighScoreManager();

    // Displays the current high score
    private JLabel highScoreLabel;

    /**
     * Constructor - creates the level selection panel.
     * 
     * @param manager Reference to the PanelManager to switch screens
     */
    public LevelSelectionPanel(PanelManager manager) 
    {
        // Set overall layout and appearance
        setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
        setBackground(new Color(180, 235, 220));
        setBorder(new EmptyBorder(40, 40, 40, 40));

        // Title 
        add(Box.createVerticalStrut(40));
        JLabel title = new JLabel("Select Difficulty");
        title.setFont(new Font("Arial", Font.BOLD, 32));
        title.setForeground(new Color(20, 60, 50));
        title.setAlignmentX(Component.CENTER_ALIGNMENT);
        add(title);
        add(Box.createVerticalStrut(30));

        // Map panel
        JPanel mapPanel = new JPanel(new GridBagLayout());
        mapPanel.setOpaque(false); // Transparent background for styling
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 10, 5, 10);
        gbc.gridy = 0;

        // Level name label
        JLabel mapLabel = new JLabel("Level 1: Heart");
        mapLabel.setFont(new Font("Arial", Font.BOLD, 20));
        mapLabel.setForeground(new Color(50, 90, 80));
        gbc.gridx = 0;
        gbc.anchor = GridBagConstraints.WEST;
        mapPanel.add(mapLabel, gbc);

        // Difficulty button names
        String[] difficulties = { "Easy", "Medium", "Hard" };

        // Create and add difficulty buttons
        for (int j = 0; j < difficulties.length; j++) {
            JButton btn = new JButton(difficulties[j]);
            styleButton(btn);

            // Set the action command in format: "LevelNumber, DifficultyNumber"
            btn.setActionCommand("1," + (j == 0 ? 0.5 : j == 1 ? 1 : 2)); // Level 1, difficulty index starting from 0.5 -> 2

            // Clicking the button starts the game
            btn.addActionListener(e -> {
                String choice = ((JButton) e.getSource()).getActionCommand();
                manager.startGame(choice);
            });

            gbc.gridx = j + 1;
            gbc.anchor = GridBagConstraints.CENTER;
            mapPanel.add(btn, gbc);
        }

        // High score label
        highScoreLabel = new JLabel("Highscore: " +
                (scoreManager.getScore(1) == Integer.MAX_VALUE ? "-" : scoreManager.getScore(1)));
        highScoreLabel.setFont(new Font("Arial", Font.PLAIN, 16));
        highScoreLabel.setForeground(new Color(80, 80, 80));
        gbc.gridx = difficulties.length + 1;
        gbc.anchor = GridBagConstraints.EAST;
        mapPanel.add(highScoreLabel, gbc);

        // Add map panel to main layout
        mapPanel.setAlignmentX(Component.CENTER_ALIGNMENT);
        add(mapPanel);
        add(Box.createVerticalStrut(15));
        add(Box.createVerticalGlue());

        // Return button
        JButton backButton = new JButton("Return");
        styleButton(backButton);
        backButton.addActionListener(e -> manager.showStartMenu());
        add(backButton);
        add(Box.createVerticalStrut(30));
    }

    /**
     * Styles a button to have a consistent look.
     * 
     * @param button The button to style
     */
    private void styleButton(JButton button) {
        button.setFont(new Font("Arial", Font.BOLD, 18));
        button.setBackground(new Color(100, 200, 180));
        button.setForeground(Color.WHITE);
        button.setFocusPainted(false);
        button.setAlignmentX(Component.CENTER_ALIGNMENT);
        button.setPreferredSize(new Dimension(140, 40));
        button.setMaximumSize(new Dimension(140, 40));
        button.setBorder(BorderFactory.createEmptyBorder(5, 15, 5, 15));
    }

    /**
     * Updates the displayed high score for Level 1.
     */
    public void updateHighScore() {
        scoreManager.loadScores();
        highScoreLabel.setText("Highscore: " +
                (scoreManager.getScore(1) == Integer.MAX_VALUE ? "-" : scoreManager.getScore(1)));
    }
}
